param(
    [string]$ApiKey = $env:INSFORGE_API_KEY,
    [string]$BaseUrl = $env:INSFORGE_BASE_URL
)

$ErrorActionPreference = "Stop"

if (-not $ApiKey) { throw "INSFORGE_API_KEY environment variable is required" }
if (-not $BaseUrl) { throw "INSFORGE_BASE_URL environment variable is required" }
...
CREATE OR REPLACE FUNCTION public.create_system_event(
  ...
  p_entity_type text,
  p_entity_id text,
  p_payload jsonb DEFAULT NULL) RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
AS $$DECLARE
  v_id uuid;
...
CREATE OR REPLACE FUNCTION public.write_frontend_audit(
  ...
  p_metadata jsonb DEFAULT NULL) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
AS $$DECLARE
  v_audit_id uuid;
